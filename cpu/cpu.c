...
// Obtener el nombre e id de usuarios
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Modulo CPU proyecto 1 Sopes 1");
MODULE_AUTHOR("Elder Navarijo");
...
static int escribir_archivo(struct seq_file *archivo, void *v) {
    for_each_process(cpu) {
        ...
        seq_printf(archivo, ",");
        seq_printf(archivo, "%u", cpu->__state);
        seq_printf(archivo, ",");
        ...
        seq_printf(archivo, "%u", from_kuid(&init_user_ns, cpu->cred->user->uid));
        ...
        list_for_each(lstProcess, &(cpu->children)) {
            ...
            seq_printf(archivo, "%u", from_kuid(&init_user_ns, child->cred->user->uid));
...
{
    proc_create("cpu_201701095", 0, NULL, &operaciones);
...
{
    remove_proc_entry("cpu_201701095", NULL);
